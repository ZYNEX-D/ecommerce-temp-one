            ...
            {isCartOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed top-0 right-0 h-full w-full max-w-lg bg-white z-50 flex flex-col shadow-[0_0_100px_rgba(0,0,0,0.2)] rounded-l-[3rem] overflow-hidden"
                    >
                        {/* Header */}
                        <div className="flex items-center justify-between p-10 border-b border-surface-100">
                            ...
                            <button
                                onClick={() => setCartOpen(false)}
                                className="w-12 h-12 flex items-center justify-center hover:bg-surface-50 rounded-2xl transition-all text-surface-400 hover:text-surface-950 border border-transparent hover:border-surface-200"
                            >
                                <X size={24} />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-10 flex flex-col gap-8 scrollbar-hide">
                            {items.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-full text-center">
                                    <div className="w-32 h-32 rounded-[2.5rem] border-2 border-dashed border-surface-200 flex items-center justify-center mb-8 bg-surface-50">
                                        <ShoppingCart className="text-surface-200" size={48} />
                                    </div>
                                ...
                                items.map((item) => (
                                    <motion.div
                                        ...
                                        key={item.id}
                                        className="flex gap-6 bg-white p-6 rounded-[2rem] border border-surface-100 group hover:border-brand-500 hover:shadow-xl transition-all duration-500 relative"
                                    >
                                        <div className="relative w-28 h-28 bg-surface-50 rounded-3xl overflow-hidden shrink-0 border border-surface-100">
                                            <Image
                                        ...
                                        <div className="flex-1 flex flex-col justify-between py-1">
                                            ...
                                            <div className="flex items-center gap-4 bg-surface-50 rounded-2xl w-fit p-1.5 border border-surface-100">
                                                <button
